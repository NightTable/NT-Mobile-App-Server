...
router.delete("/club/:clubid", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "The club was successfully deleted from the database",
    });
    return;
  } catch (err) {
    res.status(400).send({
      message:
        "Invalid request - The club was not able to be deleted from the database",
    });
    return;
  }
});
...
router.get("/:clubid", async (req, res) => {
  ...
  try {
    ...
    res.status(400).send({
      message: "Invalid request - We could not retrieve that specific club",
    });
    return;
  }
});
