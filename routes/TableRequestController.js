...
router.delete('/:tablereqid/:participantId', async (req, res) => {
    ...
    try {
        ...
        await TableRequestParticipantMapping.deleteMany({ tableReqId: new ObjectId(tableReqIdParam) });
...
router.post('/participant/:participantid', async (req, res) => {
    ...
    try {
        ...
        let participants = [];
        let orgUserParticipant = await Participant.findById(new ObjectId(organizerId)); 
        let orgUserParticipantId = orgUserParticipant.userId;
        let orgUser = await User.findById(new ObjectId(orgUserParticipantId));
        let tableConfig = await TableConfiguration.findById(new ObjectId(tableConfigIdParam)); 
        const tableConfigPrice = tableConfig.price;

        participants.push({
            tableRequestId: null,
            partId: organizerId,
            minPrice: tableConfigPrice / (participantIdsParam.length + externalEmailsParam.length),
        ...
        for (let i = 0; i < participantIdsParam.length; i++){
            ...
            let invitedParticipant =  await Participant.findById(new ObjectId(invitedPartId));    
            let invitedPartUserId = invitedParticipant.userId; 
            let invitedPartUser =  await User.findById(new ObjectId(invitedPartUserId));  
        ...
        if (tableConfig.size < participants.length){
            availableSpots = 0; 
            res.status(400).send({message: "table size is less than number of participants"});
        }
        else{
        ...
        for (let i = 0; i < participants.length; i++){
            let tableRequestParticipantMap = await TableRequestParticipantMapping.create({
                ...
                costContribution: participants[i].minPrice + participants[i].addAmount,
        ...
        res.status(400).send({message: 'Invalid request - the table request could not be created'});
        return;
    }
...
router.get('/participant/:participantid', async (req, res) => {
    ...
    try {
        ...
        const associatedParticipant = await Participant.findById(new ObjectId(participantIdParam));
        const associatedUserId = associatedParticipant.userId;

        participantMappingObjects = await TableRequestParticipantMapping.find({ participantId: new ObjectId(participantIdParam)})
        ...
        for (let i = 0; i < participantMappingObjects.length; i++){
            ...
            let localTableObjectTableConfigExpanded = await localParticipantMappingObject.tableReqId.populate("tableConfigId");
            let fullTableObjectExpanded = await localTableObjectTableConfigExpanded.populate("clubId");

            const organizerParticipantObject = (await TableRequestParticipantMapping.find({ 
                tableReqId: localParticipantMappingObject.tableReqId.id,
                isRequestOrganizer: true
            }).populate('participantId'))[0].participantId;

            const organizerUserObject = (await organizerParticipantObject.populate('userId')).userId;
            ...
            const tableReqFriendMappings = await TableRequestParticipantMapping.find({ tableReqId: fullTableObjectExpanded.id }).populate('participantId');

            for (let j = 0; j < tableReqFriendMappings.length; j++) {


                otherTableParticipants.push(tableReqFriendMappings[j].participantId.userId.toString());
            }

            const friendMappingObjects = await UserFriendMapping.find({ sourceId: associatedUserId });
            ...
            if (friendMappingObjects.length !== 0) {
                ...
                for (let j = 0; j < friendMappingObjects.length; j++) {
                    ...
                    if (otherTableParticipants.includes(friendMappingObjects[j].targetId.toString())) {
                        dynamicFriendCount++;
                    }
                }
        ...
        res.status(400).send({ message: "Invalid request - We could not find any table requests from the specified participant ID"});
    }
...
router.get('/tablereqquery', async (req, res) => {
    ...
    try {
        let tableConfigs = await TableConfiguration.find({price: {$lte: priceParam}, clubId: new ObjectId(clubIdParam), size: tableSizeParam});
        let result = []
        for (let i = 0; i < tableConfigs.length; i++){
            let tableRequests = await TableRequest.find({tableConfigId: new ObjectId(tableConfigs[i].id), mfRatio: {$gte: mfratioParam}});
        ...
        res.json({message: `Invalid request - we could not find any table requests matching the specified filter requirements`})
        return;
    }
})
