...
describe('Testing the POST /api/clubs/user/:userid endpoint', () => {
    ...
    beforeAll( async() => {
        ...
        await User.deleteOne({ userName: "dummyPostUserClubNamePostClubsByUserConsumer" });

        sampleRegion = await Region.create({
            cityArea: "FakeMiamiPostClubsByUser",
            referenceLat: 39.333,
        ...
        sampleUserConsumer = await User.create({
            firstName: "PostUserClubFirstName",
            lastName: "PostUserClubLastName",
            userName: "dummyPostUserClubNamePostClubsByUserConsumer",
            profilePhoto: "https://cache.legacy.net/legacy/images/cobrands/shreveporttimes/photos/SPT021686-1_20130820.jpgx?w=365&h=399&option=3",
            gender: "male",
            email: "dummypostuserclub@gmail.com",
            gmail: null,
        ...
        await sampleUserConsumer.save();
    });

    it(`Should return a 400 error if the user specificed in the query params id is
        the role of consumer and not manager`, async () => {
            ...
            const response = await request(app)
                .post(`/api/clubs/user/${sampleUserConsumer.id}`)
    ...
    afterAll( async () => {
        ...
        await User.deleteOne({ _id: new ObjectId(sampleUserConsumer.id) });
